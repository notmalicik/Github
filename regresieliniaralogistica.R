...
newdata = data.frame(popularity = 80)

# Obține probabilitățile prezise
predictions2 <- predict(logistic_model, newdata = test_data_logistic, type = "response")

# Calculează AUC-ROC
roc_curve <- roc(test_data$explicit, predction2)
auc_score <- auc(roc_curve)
print(auc_score)


datapr <- data.frame(popularity = seq(100, 1, by = -1))
predction2 <- predict(logistic_model, datapr)

print(predction2)

confusion_matrix <- confusionMatrix(predict(logistic_model, newdata = test_data) > 0.5, test_data$explicit)
